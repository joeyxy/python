...
def _get_SecretPrefix(phone):
    ...
    if req_status == 0:
        ...
        print "phone:%s SecretPrefix:%s" % (phone[:8],SecretPrefix)
        return SecretPrefix
    elif req_status == 4004 or req_status == 4003:
        print "faild send sms code at phone:%s" % phone[:8]
        sys.exit(1)
...
def _check(args):
    ...
    try:
        ...
        req_status = 4014
    if req_status == 0:
        print "phone:%s done at code:%s,passwrod:%s" % (phone[:8],code,password)
