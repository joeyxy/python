...
#get the list from zoomeye,the use the ssl script test ip then gather the ip list by country.
...
class ssl_test(object):
    def __init__(self,country,port,sid,eid):
    ...
    def getip(self):
        for i in range(self.sid,self.eid):
            url = "http://www.zoomeye.org/lab/heartbleed/2015?port=%s&code=%s&p=%s" % (self.port,self.country,i)
            try:
                req = requests.get(url,timeout=3)
                #print req.content
                pattern = re.compile('<td>(\d+\.\d+\.\d+\.\d+)</td>', re.S)
    ...
    def write_file(self):
        while queue.qsize() > 1:
            ip = queue.get()
            file_name = "%s_%s_ssl_vulnerable_ip.list" % (self.country,self.port)
            f = open(file_name, 'a+')
            f.write('%s\n'%ip)
            f.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--country',action="store",dest="country",default="CN")
    parser.add_argument('--port',action="store",dest="port",type=int,default="443")
    parser.add_argument('--sid',action="store",dest="sid",type=int,default="1")
    parser.add_argument('--eid',action="store",dest="eid",type=int,default="2")
    given_args = parser.parse_args()
    test=ssl_test(country=given_args.country,port=given_args.port,sid=given_args.sid,eid=given_args.eid)
    test.getip()
    test.start()
    #test.write_file()