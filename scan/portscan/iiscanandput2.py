...
def options(i, iq, oq):
	while True:
		try:
			...
			if result == []:
				print colored("On bad.%s.the web DAV is not open.\n" % ip,'red')
			else:       
				print colored("WA HAHA LET US CHECK MORE time:%s" % ip,'green') 
				oq.put(ip)
			iq.task_done()
		except socket.error,e:
				#print colored("cannot connect ip:%s failed:%s" % (ip,e),'blue')
				iq.task_done()
...
if in_queue.qsize() < num_options_threads:
	num_options_threads = in_queue.qsize()


#spawn pool of optins threads
for i in range(num_options_threads):
    worker = Thread(target=options, args=(i, in_queue,out_queue))
    worker.setDaemon(True)
    worker.start()
	
#spawn pool of put threads
for i in range(num_put_threads):
